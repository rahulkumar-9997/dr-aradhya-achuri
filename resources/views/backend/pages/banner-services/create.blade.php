...
<div class="content">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('banner-services.store') }}" method="POST" enctype="multipart/form-data" id="addBannerServiceForm">
            @csrf
            <div class="row">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="select2 form-control @error('services_multiple.*') is-invalid @enderror"
                            ...
                            <option value="{{ $service->id }}"
                                {{ (old('services_multiple') && in_array($service->id, old('services_multiple'))) ? 'selected' : '' }}>
                                {{ $service->title }}
                            </option>
                            @endforeach
                            @endif
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="summer-description-box mb-3">
                        ...
                        <textarea class="ckeditor4 form-control @error('content') is-invalid @enderror" name="content"
...
<script>
    $(document).ready(function() {
        ...
        $('#addBannerServiceForm').on('submit', function() {
            var submitButton = $('#submitButton');
            var submitText = $('#submitText');
            submitButton.prop('disabled', true);
            submitText.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
        });
    });
</script>
@endpush